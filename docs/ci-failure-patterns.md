...
infra. Before pushing language / sem / examples / extension changes, run the
matching checks below.

## Quick local gate (language / sem)
...
### 3. Extension package version / npm test pin

| Symptom | Extension workflow or `npm test` fails on version mismatch / stale pin |
| Cause | `package.json` version bumped (or not) while tests still assert an old version, or tag ≠ `extension-v*` tip |
| Prevent | Bump version + `RELEASE_NOTES.md` together; do not hard-pin the version string in tests that must track trunk; tag only `extension-v<version>` from `main` tip |
| Related | CER-001 §8; `pys-language/PUBLISH.md` |
